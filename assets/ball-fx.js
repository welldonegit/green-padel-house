(function(){
  ...
  var LIFE = 1000, PER_VOLLEY = 3, COOLDOWN = 1000;
  var last = null;

  function spawn(btn){
    // hard global cap: while any ball is in flight, no new volley (protects against duplicate listeners)
    var now = Date.now();
    if (window.__gphVolleyUntil && now < window.__gphVolleyUntil) return;
    window.__gphVolleyUntil = now + COOLDOWN;

    var r = btn.getBoundingClientRect();
    for (var i = 0; i < PER_VOLLEY; i++){
      ...
      (function(el){ setTimeout(function(){ if (el.parentNode) el.parentNode.removeChild(el); }, LIFE); })(ball);
    }
  }
